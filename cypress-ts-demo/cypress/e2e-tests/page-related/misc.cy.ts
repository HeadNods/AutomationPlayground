import { MiscPage } from '../../page-object-model/index';
...
describe('Misc with Page Objects', () => {
  ...
  it('cy.exec() - execute a system command', () => {
    ...
    miscPage
      .executeSystemCommand('echo Jane Lane')
      .its('stdout')
    ...
    if (Cypress.platform === 'win32') {
      miscPage
        .executeSystemCommand(`print ${Cypress.config('configFile')}`)
        .its('stderr')
        .should('be.empty');
    } else {
      miscPage
        .executeSystemCommand(`cat ${Cypress.config('configFile')}`)
        .its('stderr')
        .should('be.empty');
