import { ActionsPage } from '../../page-object-model/index';
...
describe('Actions with Page Objects', () => {
  ...
  it('.check() - check a checkbox or radio element', () => {
    ...
    actionsPage
      .checkSpecificRadioButton('radio1')
      .getAllEnabledRadioButtons()
      .get("[value='" + 'radio1' + "']")
      .should('be.checked');

    // .check() accepts an array of values
    const multipleCheckboxes = actionsPage
      .checkSpecificCheckboxes(['checkbox1', 'checkbox2'])
      .getMultipleCheckboxes();
    multipleCheckboxes.get("[value='checkbox1']").should('be.checked');
    multipleCheckboxes.get("[value='checkbox2']").should('be.checked');
  ...
  it('.select() - select an option in a <select> element', () => {
    ...
    actionsPage.getSelectMultipleOption()
      .invoke('val')
  ...
  it('cy.scrollTo() - scroll the window or element to a position', () => {
    ...
    actionsPage.scrollToPosition(actionsPage.getScrollableHorizontal(), 'right');

    // or you can scroll to a specific coordinate:
    // (x axis, y axis) in pixels
    actionsPage.scrollToCoordinates(actionsPage.getScrollableVertical(), 250, 250);

    // or you can scroll to a specific percentage
    // of the (width, height) of the element
    actionsPage.scrollToPercentage(actionsPage.getScrollableBoth(), '75%', '25%');

    // control the easing of the scroll (default is 'swing')
    actionsPage.getScrollableVertical()
      .scrollTo('center', { easing: 'linear' });

    // control the duration of the scroll (in ms)
    actionsPage.getScrollableBoth()
