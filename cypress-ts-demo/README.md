...
The workflow is defined in `.github/workflows/cypress-ts-workflow.yml` and provides:
...
- Only triggers when changes are made to:
  - `cypress-ts-demo/**` (any files in this directory)
  - `.github/workflows/cypress-ts-workflow.yml` (the workflow file itself)

**Pull Request Events:**
- Runs on PRs targeting `main` or `master` branches
- Only triggers when changes affect `cypress-ts-demo/**`
...
2. Installs dependencies from `cypress-ts-demo/package.json`
...
- Path: `cypress-ts-demo/cypress/screenshots`
- Retention: 3 days
- Available under workflow run artifacts as `cypress-screenshots`

**Videos** (always):
- Path: `cypress-ts-demo/cypress/videos`
- Retention: 3 days
- Available under workflow run artifacts as `cypress-videos`

**Test Reports** (always):
- Path: `cypress-ts-demo/cypress/reports/*.html`
- Retention: 5 days
- Available under workflow run artifacts as `cypress-test-results`

### Customizing the Workflow

To modify when tests run, edit `.github/workflows/cypress-ts-workflow.yml`:
...
on:
  push:
    ...
    paths:
      - 'cypress-ts-demo/**'        # Change path filters
```
...
The main configuration is in `cypress.config.ts`. Key settings include:
